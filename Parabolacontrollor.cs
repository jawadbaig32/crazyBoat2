...
public class Parabolacontrollor : MonoBehaviour
{
    /// <summary>
    /// Animation Speed
    /// </summary>
   ...
   public void DrawPathLine()
      ...
      if ((gizmo.Points.Length - 1) % 2 != 0)
        ...
        for (int c = 1; c <= accur; c++)
        {
           ...
           // Gizmos.DrawLine(prevPos, currPos);
            ...
            prevPos = currPos;
            if(spheres[c - 1]!=null)
            spheres[c - 1].transform.position = currPos;
    ...
    { 
        ...
        //   Instantiate(spherePrefab.gameObject,transform);


    }

    public void OffPathLine()
    {
    ...
    void Spawnsphere()
    {
        spheres = new GameObject[accur];
        for (int a = 0; a < accur; a++)
        {
            spheres[a] = Instantiate(spherePrefab, sphereParent);
...
void Update()
    ...
    public void FollowParabola()
    {
        ...
        for (int i=0; i<=parabolaFly.Points.Length-1; i++) {
            transform.LookAt(parabolaFly.Points[i].position);
        }
            Animation = true;
    ...
    public void LookAtNext()
    {
        ...
        for (int a = 0; a < accur; a++)
        {
            if (!(a > accur - 2))
                ...
                spheres[a].transform.LookAt(spheres[a + 1].transform);
            spheres[accur - 1].transform.rotation = spheres[accur - 2].transform.rotation;
